...
The `features` option on `markdownToHtml`, `mdxToJs`, `markdownToJs`, `markdownToHast`, and `mdxToHast` toggles which Markdown extensions the parser recognizes (see [Entry points](/docs/entry-points/) for those functions). By default, Sätteri enables `gfm` and `frontmatter`.

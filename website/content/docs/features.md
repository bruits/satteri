...
The `features` option on `markdownToHtml`, `mdxToJs`, `markdownToHast`, and `mdxToHast` toggles which Markdown extensions the parser recognizes. By default, Sätteri enables `gfm` and `frontmatter`.

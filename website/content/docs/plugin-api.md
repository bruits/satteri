...
`setField` writes a scalar field of the node itself. `key` must be a supported string, number, boolean, or nullable scalar field of the node type, and `value` must match that field's type, on both MDAST and HAST. This is how you rename a node: `setField(node, "tagName", "App.Link")` turns a HAST `<a>` into `<App.Link>`, and `setField(node, "name", "warning")` renames an MDX JSX element or a directive. An MDX JSX `name` also accepts `null` to create a fragment. `children` and `data` have dedicated `setField` overloads; `data` is an open bag serialized to JSON, and `null` clears it. Other container-shaped fields use their dedicated setter (`setAttribute` or `setProperty`) or a full `replaceNode`; for example, replace a GFM table node to change its `align` array.

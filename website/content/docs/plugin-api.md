...
- **A few contradictory combinations throw.** Replacing a node with new content that reuses that same node while another plugin edits something inside it in the same pass, two replacements that each reuse the other's node, and inserting a sibling next to the root. Two more come from reuse having no answer, and both report at the call that caused them: inserting a node into a position it contains (`insertAfter(node, ctx.parent(node))`), and two inserts that each reuse the other's node. Reordering siblings is the case that runs into the second one, and the way to write it is to hand the parent the order you want rather than to shuffle by pairwise inserts:

```js
ctx.setProperty(parent, "children", [...parent.children].sort(byWhatever));
``` Replacing, removing, or wrapping the root itself — say, via `ctx.parent()` on a top-level node — works fine.

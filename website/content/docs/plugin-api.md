...
- **Stale node references fail the compile.** A transform anchored on a node id that no longer exists — typically a node object cached from a previous document or a previous pipeline stage — throws an `invalid node id` error instead of being silently dropped. Retain nodes within a pass freely; don't carry them across documents.
- **A few pathological shapes throw `unsupported patch shape`.** Most notably: replacing a node with content that reuses the node itself while another transform in the same pass edits one of its descendants, circular reuse between two replacements, and inserting a sibling before or after the root (the root has no siblings). Replacing, removing, or wrapping the root itself — for example via `ctx.parent()` on a top-level node — is supported.

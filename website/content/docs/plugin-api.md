...
Node objects are shared, not copied: the same underlying node reached through any path (`children`, `ctx.parent()`, a later pass over an unchanged tree) is the same JavaScript object. Treat nodes as read-only data and go through the context methods for changes; a local mutation of a node object is visible to every later plugin that sees that node, without ever being applied to the tree.

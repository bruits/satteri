...
fn empty_sublist_cant_interrupt_paragraph() {
    ...
    let (arena, _) = parse(input, DEFAULT_OPTIONS);
    let root_children = arena.get_children(0);
    let types: Vec<u8> = root_children.iter().map(|&id| arena.get_node(id).node_type).collect();
    assert_eq!(root_children.len(), 2, "expected paragraph + list, got types {:?}", types);
    ...
    assert_eq!(item_children.len(), 1, "item should have 1 child (paragraph, not sublist)");
}
...
fn mdx_expression_then_jsx_is_flow() {
    ...
    eprintln!("MdxFlowExpression = {}", MdastNodeType::MdxFlowExpression as u8);
    eprintln!("MdxJsxFlowElement = {}", MdastNodeType::MdxJsxFlowElement as u8);
    assert!(root_children.len() == 2, "expected 2 flow nodes, got {} with types {:?}", root_children.len(), types);
}
...
fn mdx_fragment_with_content_is_flow() {
    ...
    let types: Vec<u8> = root_children.iter().map(|&id| arena.get_node(id).node_type).collect();
    eprintln!("types: {:?}", types);
    assert_eq!(root_children.len(), 1, "expected 1 flow element, got {} with types {:?}", root_children.len(), types);
}
...
fn blockquote_then_dash_after_para() {
    ...
    assert_eq!(root_children.len(), 3, "expected 3 children, got {:?}", types);
    assert_eq!(arena.get_node(root_children[2]).node_type, MdastNodeType::List as u8);
}
...
fn empty_list_in_blockquote_after_para() {
    ...
    assert_eq!(bq_child_type, MdastNodeType::Paragraph as u8,

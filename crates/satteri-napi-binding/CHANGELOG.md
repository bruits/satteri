...
- Updated dependencies: satteri-ast (Cargo)@0.5.2, satteri-mdxjs (Cargo)@0.3.11, satteri-plugin-api (Cargo)@0.5.2, satteri-pulldown-cmark (Cargo)@0.6.2
...
- [1b09752](https://github.com/bruits/satteri/commit/1b09752d04b7e5494b583250330089a6401b377d) Added `{ fragment: true }` to `htmlToHast`, which parses the string as a fragment so the returned `root` holds its own top-level nodes instead of an implied `<html>`/`<head>`/`<body>`.
  ...
  ```
   — Thanks @Princesseuh!
- Updated dependencies: satteri-ast (Cargo)@0.5.1, satteri-mdxjs (Cargo)@0.3.10, satteri-plugin-api (Cargo)@0.5.1, satteri-pulldown-cmark (Cargo)@0.6.1

...
fn main() {
    ...
    let dirs = [
        &mdast_rs, &hast_rs, &ast_rs, &arena_rs, &plugin_rs, &napi_rs, &mdast_ts, &hast_ts,
        &shared_ts,
    ];
    for dir in dirs {
    ...
    write(
        &arena_rs.join("remap_refs.rs"),
        &emit::remap_refs_rs(&mdast_layouts, &mdast_tails, &hast_layouts, &hast_tails),
    );
